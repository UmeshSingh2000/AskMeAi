...
export default function PdfUploader() {
  ...
  const handleUpload = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/user/uploadpdf`,
        formData,
        {
          headers: {
  ...
  return (
    <div id="pdf-uploader" className="relative flex flex-col items-center justify-center p-8 rounded-xl border-2 border-dashed border-muted-foreground/30 bg-background mx-auto">
      {/* 🔒 Overlay when not logged in */}
      {!isLoggedIn && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 backdrop-blur-sm bg-black/50 rounded-xl">
          <p className="text-white mb-4 text-lg font-medium">
            Please login to upload your PDF
          </p>
          <LoginForm />
        </div>
      )}
      ...
      {/* Upload Area */}
      <div className={`${!isLoggedIn ? "blur-md pointer-events-none select-none" : ""} w-full`}>
        {!previewUrl ? (
          <label
          ...
          >
            ...
            <Button
              ...
              className="bg-purple-600 hover:bg-purple-700 cursor-pointer text-white mb-2">
